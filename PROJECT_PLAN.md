...
| **Documentation** | **7.0/10** | README, PROJECT_PLAN, INSTALL, EXAMPLES all written and aligned with v2.2.0 surface; honest scorecard at the top of each; `stealth::version()` returns `"2.2.0"` reflecting shipped API surface | Doxygen-style per-symbol contracts not yet produced; ~5 `static_assert`s in `stealth.hpp` covering literal-length, build-key non-zero, integral properties |
...
* xorstr `include/xorstr.hpp` was fetched from the canonical repo
  ...
  **242 lines, 10 KB** total, **2 public macros**, **5 noteworthy design
  ...
  Feature grid (competitor vs. stealthlib v2.2.0):

  | Feature                                  | xorstr | stealthlib v2.2.0 |
  |------------------------------------------|:------:|:----------------:|
